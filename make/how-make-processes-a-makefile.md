...
Suppose we modify `intmath.c` but leave `testintmath.c` and `intmath.h` untouched. This renders `intmath.o` and `testintmath` out-of-date. Here's how `make` updates them:
